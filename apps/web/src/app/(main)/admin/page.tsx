...
export default function AdminPage() {
  ...
  const { data: logsData, isLoading: logsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.data as { id: string; userId: string | null; action: string; details: any; ipAddress: string | null; createdAt: string }[];
    },
        ...
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          ...
          {usersLoading ? (
            ...
            <div className="divide-y divide-border">
              {(usersData || []).map((u) => (
                <div key={u.id} className="grid grid-cols-12 gap-4 p-4 text-sm items-center hover:bg-muted/20 transition-colors">
                  <div className="col-span-4 font-medium text-foreground">{u.name || '—'}</div>
                  <div className="col-span-4 text-muted-foreground font-mono text-xs">{u.email}</div>
        ...
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          ...
          {logsLoading ? (
            ...
            <div className="divide-y divide-border max-h-[600px] overflow-y-auto">
              {(logsData || []).map((log) => (
                <div key={log.id} className="grid grid-cols-12 gap-4 p-4 text-sm items-center hover:bg-muted/20 transition-colors">
                  <div className="col-span-3">
                    <StatusBadge
                      variant={
                        log.action.includes('login') ? 'success' :
                        log.action.includes('failed') ? 'error' :
                        log.action.includes('delete') || log.action.includes('REVOKE') ? 'error' :
                        'default'
                      }
                    >
                      {log.action}
                  ...
                  <div className="col-span-3 text-muted-foreground font-mono text-xs truncate">{log.userId || '—'}</div>
                  <div className="col-span-2 text-muted-foreground text-xs">{log.ipAddress || '—'}</div>
                  <div className="col-span-4 text-muted-foreground text-xs">{new Date(log.createdAt).toLocaleString()}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {healthLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="rounded-xl border border-border bg-card p-6 text-center">
                <Activity className={cn('mx-auto h-8 w-8 mb-2', healthData.status === 'healthy' ? 'text-green-500' : 'text-red-500')} />
                <p className="text-sm font-semibold text-foreground capitalize">{healthData.status}</p>
                <p className="text-xs text-muted-foreground mt-1">System Status</p>
              </div>
              <div className="rounded-xl border border-border bg-card p-6 text-center">
                <div className={cn('mx-auto h-8 w-8 rounded-full mb-2 flex items-center justify-center', healthData.dbConnected ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500')}>
                  {healthData.dbConnected ? '✓' : '✗'}
                </div>
                <p className="text-sm font-semibold text-foreground">{healthData.dbConnected ? 'Connected' : 'Disconnected'}</p>
                <p className="text-xs text-muted-foreground mt-1">Database</p>
              </div>
              <div className="rounded-xl border border-border bg-card p-6 text-center">
                <Clock className="mx-auto h-8 w-8 mb-2 text-primary" />
                <p className="text-sm font-semibold text-foreground">{Math.floor(healthData.uptime / 3600)}h {Math.floor((healthData.uptime % 3600) / 60)}m</p>
