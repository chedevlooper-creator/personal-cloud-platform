...
export function WorkspaceIdentityBar({ user }: { user?: AuthUser }) {
  ...
  return (
    <div className="pointer-events-none absolute inset-x-4 bottom-3 flex justify-center">
      <div className="flex max-w-full flex-wrap items-center justify-center gap-x-3 gap-y-1 rounded-full px-3 py-2 text-xs text-muted-foreground md:gap-5 md:text-sm">
        {items.map((item) => (
